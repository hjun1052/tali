...
The pushed tag must match the version in `Cargo.toml`. For example, `Cargo.toml` version `0.1.0` must be released with tag `v0.1.0`.
...
Use the `Publish crate` GitHub Actions workflow and enter the exact version from `Cargo.toml`, for example `0.1.0`.
